    ...
    class Eatery
    {
            ...
            if (GameManager.GameInfo.Pack.GoodsList.ContainsKey(goods) && GameManager.GameInfo.Hero.Money - goods.Money >= 0)
            {
                for (int i = 0; i < GameManager.GameInfo.Pack.GoodsList.Count; i++)
                {
                    ...
                    if (((Goods)GameManager.GameInfo.Pack.GoodsList[i]).Name.Equals(goods.Name))
                    {
                        ...
                        GameManager.GameInfo.Pack.GoodsList[goods] = ((int)GameManager.GameInfo.Pack.GoodsList[goods]) + 1;		//包裹物品数量加1
                        return true;
                    }
                }
            }
        ...
        public bool Buy(Goods goods)
		{
			for(int i = 0; i < GameManager.GameInfo.Pack.GoodsList.Count; i++)
			{
				...
				if(GameManager.GameInfo.Pack.GoodsList.ContainsKey(goods) && ((int)GameManager.GameInfo.Pack.GoodsList[goods]) > 0)
				{
					GameManager.GameInfo.Hero.Money += goods.SaleMoney;		//增加物品出售金额
					GameManager.GameInfo.Pack.GoodsList[goods] = ((int)GameManager.GameInfo.Pack.GoodsList[goods]) - 1;		//包裹物品数量减1
					return true;
				}
			}
			return false;
